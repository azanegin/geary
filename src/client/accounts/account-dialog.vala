...
public class AccountDialog : Gtk.Dialog {
    ...
    private void on_save_add_or_edit(Geary.AccountInformation info) {
        ...
        if (add_edit_pane.get_mode() == AddEditPage.PageMode.EDIT && info.is_copy()) {
            ...
            if (real_info != null) {
                validate_connection = !real_info.imap_information.credentials.equal_to(info.imap_information.credentials) ||
                    (info.smtp_information.credentials != null && !real_info.smtp_information.credentials.equal_to(info.smtp_information.credentials));
            }
        }
